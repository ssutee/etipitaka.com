...
def generate_books
  for lang in ['thai', 'pali'] do
    File.open(File.join('.','db',lang+'_name.txt')) do |infile|
      while line = infile.gets
        volume = line.strip.split[0].to_i
        title = line.strip.split[(1..-1)].join(' ')
        if lang == 'thai' or lang == 'pali'
          if volume <= 8
            category = 1
          elsif volume <= 33
            category = 2
        ...
        Book.create(:language => lang, :category => category, :volume => volume, :title => title)
      end
    end
  end
end
